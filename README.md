...
The fourteen tool definitions come to about 12,000 tokens. Clients that load tool definitions on demand (Claude Code among them) pay only for the tools a conversation uses. On a small local model with a short context window, enable only the tools you need.
